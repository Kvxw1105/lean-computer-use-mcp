...
class LlmRefiner:
    ...
    def refine(self, library: MemoryLibrary) -> RefineSuggestions:
        if not self.api_base or not self.api_key or not self.model:
            raise ValueError(
                "memory refinement requires api_base, api_key and model "
                "(set LEAN_CU_VISION_API_BASE / LEAN_CU_VISION_API_KEY / LEAN_CU_VISION_MODEL "
                "or pass --api-base/--api-key/--model)"
            )
        import httpx
        ...
        payload = {
            ...
            "messages": [
                ...
                {"role": "user", "content": library_digest(library)},
            ],
        }
        ...
        except Exception as exc:  # noqa: BLE001 - surfaced as a refine failure
            raise ValueError(f"memory refinement request failed: {exc}") from exc
